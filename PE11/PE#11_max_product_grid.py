# -*- coding: utf-8 -*-
"""
Created on Thu Jun  1 12:23:56 2017
...
#Disecting grid such that matrix M contains all elements in form M[row][column]
#M = [i.split() for i in grid]
#M = [[int(j) for j in i] for i in M]

"METHOD 2 - Calling a file"

with open('PE11grid.txt') as f:
    M = []
    for line in f:
...
for i in range(20):
    for j in range(16):
        #vertical products
        prod_vert=M[i][j]*M[i][j+1]*M[i][j+2]*M[i][j+3]
        if prod_vert>prod_max:
            prod_max=prod_vert
        #horiszontal products
        prod_horiz=M[j][i]*M[j+1][i]*M[j+2][i]*M[j+3][i]
        if prod_vert>prod_max:
            prod_max=prod_vert
...
for i in range(3,20):
    for j in range(0,16):
        diag_pos=M[i][j]*M[i-1][j+1]*M[i-2][j+2]*M[i-3][j+3]
        if diag_pos>prod_max:
            prod_max=diag_pos
            
#negative diagonal
for i in range(0,16):
    for j in range(0,16):
        diag_neg=M[i][j]*M[i+1][j+1]*M[i+2][j+2]*M[i+3][j+3]
        if diag_neg>prod_max:
            prod_max=diag_neg
            

elapsed = (time.time() - start)
 
print ("The maximum product of 4 numbers in the 20x20 grid is %s found in %s seconds" % (prod_max,elapsed))